...
class DeepLabV3(nn.Module):
    def __init__(
        ...
        self.classes = classes  #15-1 step0:[15]
...
#x[8,3,512,512]
    def forward(self, x, ret_intermediate=False):
        ...
        x_b, x_pl, attentions = self.forward_before_class_prediction(x)  #x_b:[8,2048,32,32]  x_pl:[8,256,32,32]
        sem_logits_small = self.forward_class_prediction(x_pl) #[8,16,32,32]  #x_pl = self.aspp(x_b):  backbone aspp   
        ...
        if ret_intermediate:
            ...
            return sem_logits, {'neg_reg': sem_neg_logits_small, 'pos_reg': sem_pos_logits_small}, x_pl

        else:
            return sem_logits,x_pl     
    ...
    #x_pl = self.aspp(x_b)  sppa中: output=self.aspp(f) output=self.pre_classifier(output)
