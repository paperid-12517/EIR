...
from data_loader.task import get_task_labels, get_per_task_classes,get_tasks
...
class VOCIncrementalDataLoader():
    def __init__(self, task, train, val, test, num_workers, pin_memory, memory=None):
        ...
        self.classlist = [get_tasks('voc', task['name'], task['step']) for step in range(task['step']+1)]
        ...
        if val['cross_val'] is True:
            ...
            val_len = len(self.train_set) - train_len   
    ...
    def task_info(self):
        return {"setting": self.setting, "name": self.name, "step": self.step,
                "old_class": self.classes_idx_old, "new_class": self.classes_idx_new, "classlist":self.classlist}
