...
def main_worker(gpu, ngpus_per_node, config):
    ...
    logger.info(f"Old Classes: {old_classes}")  #old classes、new classes
    ...
    if config['multiprocessing_distributed']:
        if gpu is not None:
            if model_old is not None:
                model_old.to(device)
                model_old = DDP(model_old, device_ids=[gpu])
        else:
            if model_old is not None:
                model_old.to(device)
                model_old = DDP(model_old)
    else:
        if model_old is not None:
            model_old = nn.DataParallel(model_old, device_ids=device_ids)
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if model_old is not None:
            with torch.no_grad():
                logit_old, features_old = model_old(data['image'], ret_intermediate=True)
            
                pred_prob = torch.sigmoid(logit_old).detach()
                pred_scores, pred_labels = torch.max(pred_prob, dim=1)

                pseudo_labels = torch.where(
                    (data['label'] <= 0) & (pred_labels > 0) & (
                                pred_scores >= config['hyperparameter']['pseudo_thresh']),  
                    pred_labels,
                    data['label'])
        ...
        for b in range(data['image'].size(0)):
            pred_label = pred_labels[b]
            label = data['label'][b]               
            pseudo_label = pseudo_labels[b]
            true_bg_label = data["true_label"][b]   
            image = data['image'][b]
            parts = data["image_name"][b].split(' ')
            numbers = re.findall(r'\d+', parts[0])
            result = ''.join(numbers)
            img_name = result

            draw_pict.draw_pic(pred_label, label, pseudo_label,true_bg_label,image, img_name, save_path)
...
if __name__ == '__main__':
    ...
    options = [
        ...
        #CustomArgs(['--multiprocessing_distributed'], target='multiprocessing_distributed'),
